...
//global variable to keep track of class name
...
static ast lookup_variable(Token identifier, bool check)
{
...
//function to declare variables and add them to the symbol table
static ast declare_variable(Token identifier,Token type)
{
    scope current = scopes->back() ;

    //for argument variables in method calls, the offset starts at 1
    //check for argument segment and for method/function call
    ast meow = lookup_variable(identifier, true) ;

    if ( current.segment == "argument" && meow == nullptr)
    {
        if ( scopes->back().offset == 0 )
        {
            scopes->back().offset++ ;
        }
    }
    ...
    if ( !insert_variables(current.symbol_table,varname,var) ) 
    {
        fatal_error(0,"\n" + token_context(identifier) + "Variable:  " + varname +  " has already been declared") ;
    }

    return create_var_dec(var.name,var.segment,var.offset, var.type) ;
}
...
ast parse_class()
{
    ...
    //class node and subroutine nodes
    ast decs = nullptr ;
    ast subrs = nullptr ;
    ...
    ast _class = create_class(myclassname, decs, subrs) ;
    pop_error_context() ;
    return _class;
}
...
ast parse_class_var_decs()
{
    ...
    ast class_var_decs = create_class_var_decs(decs) ;
...
ast parse_static_var_dec()
{
    ...
    // vector to store static var decs
    vector<ast> decs ;

    //symbol table for static variables
    ...
    var_dec = declare_variable(name, type) ;
...
ast parse_field_var_dec()
{
    ...
    // vector to store field variable declarations
    vector<ast> decs ;

    //symbol table for fields variables
    push_symbol_table("this") ; 
    ...
    mustbe(tk_semi) ;
    ast var_decs = create_var_decs(decs) ;
    pop_error_context() ;
    return var_decs ;
}
...
ast parse_subr_decs()
{
    ...
    //vector of subroutine declarations
    vector<ast> ast_subrs ; 
    ...
    {
        if ( token_kind() == tk_constructor ) 
        {
        node = parse_constructor() ;
        ast_subrs.push_back(create_subr(node) ) ;
        }
    ...
    ast ast_subrs_decs = create_subr_decs(ast_subrs) ;
    pop_error_context() ;
    return ast_subrs_decs ;
}
...
ast parse_constructor()
{
    ...
    ast ast_const = create_constructor(token_spelling(vtype), token_spelling(name), params, body) ;
    pop_error_context() ;
    return ast_const ;
}
...
ast parse_function()
{
    ...
    pop_error_context() ;
    return create_function(token_spelling(vtype), token_spelling(name), params, body) ;
}
...
ast parse_method()
{
    ...
    pop_error_context() ; 
    return create_method(token_spelling(vtype), token_spelling(name), params, body) ;
}
...
ast parse_subr_body()
{
    ...
    //variable declarations and statement nodes
...
ast parse_var_decs()
{
    ...
    //vector of var declarations
    vector<ast> var_decs ;

    while ( have(tk_var) )
    {
        var_decs.push_back(parse_var_dec()) ;
    }
    pop_error_context() ;
    return create_var_decs(var_decs) ;
}
...
ast parse_statement()
{
    ...
    {
    case tk_let:
        statement = parse_let() ; break ;
    case tk_if:
        statement = parse_if() ; break ;
    case tk_while:
        statement = parse_while() ; break ;
    case tk_do:
        statement = parse_do() ; break ;
    case tk_return:
        statement = parse_return() ; break ;
    default: did_not_find(tg_starts_statement) ;
    }
...
ast parse_while()
{
    ...
    //ast node for condition and body
...
ast parse_term()
{
    ...
    case tk_not:
        ...
        term = create_unary_op(op, term) ;
        break ;
    default: did_not_find(tg_starts_term) ;
    }
...
ast parse_var_term()
{
    ...
    ast var = lookup_variable(id, true) ;
    string name ;

    //subroutine and index node
    ast subr_call = nullptr ;
    ast index = nullptr ;

    //check if it is an id call, call or index
    switch ( token_kind() )
    {
    case tk_lrb:
        subr_call = create_subr_call(token_spelling(id), parse_call()) ;
        return create_call_as_method(myclassname, create_this(), subr_call) ;
        break ;
    case tk_lsb:
        index = parse_index() ;
        return create_array_index(var, index) ;
        break ;
    //if variable is not defined, it is a method call
    case tk_stop:
        if ( var == nullptr )
        {
            subr_call = parse_id_call() ;
            return create_call_as_function(token_spelling(id), subr_call) ;
        } 
        else 
        {
            subr_call = parse_id_call() ;
...
ast parse_expr_list()
{
    ...
    pop_error_context() ;
    return create_expr_list(expr_list);
}
...
ast parse_infix_op()
{
    ...
    {
    case tk_add:
        infix_op = mustbe(tk_add) ; break ;
    case tk_sub:
         infix_op = mustbe(tk_sub) ; break ;
    case tk_times:
        infix_op = mustbe(tk_times) ; break ;
    case tk_divide:
         infix_op = mustbe(tk_divide) ; break ;
    case tk_and:
        infix_op = mustbe(tk_and) ; break ;
    case tk_or:
         infix_op = mustbe(tk_or) ; break ;
    case tk_lt:
        infix_op = mustbe(tk_lt) ; break ;
    case tk_gt:
         infix_op = mustbe(tk_gt) ; break ;
    case tk_eq:
         infix_op = mustbe(tk_eq) ; break ;
    default: did_not_find(tg_infix_op) ;
    }

    pop_error_context() ;
    return create_infix_op(token_spelling(infix_op));
}
