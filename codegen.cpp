...
//global variable to keep track of number of field variables 
static int field_vars = 0 ;

//global varibale to store the class name
...
void walk_class_var_decs(ast t)
{
    ...
    //walk class variable declarations
...
void walk_var_dec(ast t)
{
    ...
    //keep track of the number of field variables 
    if (segment == "this")
    {
        field_vars++ ;
    }
...
void walk_constructor(ast t)
{
    ...
    int locals = size_of_var_decs(var_decs) ;

    write_to_output("function " + myclassname + "." + name  + " " + to_string(locals) + "\n") ;
    write_to_output("push constant " + to_string(field_vars) + "\n") ;
...
void walk_subr_body(ast t)
{
    ...
    //reset the counters to 0 in each subroutine
    if_counter = 0 ;
    while_counter = 0 ;
...
void walk_expr(ast t)
{
    ...
    //walk terms and infix ops in expression
...
void walk_string(ast t)
{
    ...
    //append string letters onto string
    for (int i = 0; i<_constant.length(); i++)
    {
        char letter = _constant.at(i) ;
        write_to_output("push constant " + to_string(letter) + "\n") ;
...
void walk_bool(ast t)
{
    ...
    if ( _constant == true ) write_to_output("not\n") ;
}
...
void walk_unary_op(ast t)
{
    ...
    //translate op to jack
    if ( uop == "-" ) write_to_output("neg\n") ;
    else if ( uop == "~") write_to_output("not\n") ;
}
...
void walk_call_as_function(ast t)
{
    ...
    ast expr = get_subr_call_expr_list(subr_call) ;
    int locals = size_of_expr_list(expr) ;
    walk_subr_call(subr_call) ;

    write_to_output("call " + class_name + "." + subr_name + " " + to_string(locals) + "\n") ;
}
...
void walk_call_as_method(ast t)
{
    ...
    write_to_output("call " + class_name + "." + subr_name + " " + to_string(locals + 1) + "\n") ;
}
...
void walk_infix_op(ast t)
{
    ...
    else if ( op == "&" ) write_to_output("and\n") ;
    else if ( op == "|" ) write_to_output("or\n") ;
    else if ( op == "<" ) write_to_output("lt\n") ;
    else if ( op == ">" ) write_to_output("gt\n") ;
    else if ( op == "=" ) write_to_output("eq\n") ;
    else if ( op == "-" ) write_to_output("sub\n") ;
    else if ( op == "-" ) write_to_output("sub\n") ;
}
