...
ast copy_class_var_decs(ast t)
{
    ...
    ann class_ann = get_ann(t) ;
    if ( ndecs != 0 ) class_ann = add_ann_comments(class_ann, "private:") ;
    if ( copied ) return t ;

    return create_class_var_decs(class_ann,decs) ;
}
...
ast copy_subr_decs(ast t)
{
    ...
    ann subr_a = get_ann(t) ;
    if ( size != 0 ) subr_a = add_ann_comments(subr_a, "public:") ;
    if ( copied ) return t ;

    return create_subr_decs(subr_a,decs) ;
}
